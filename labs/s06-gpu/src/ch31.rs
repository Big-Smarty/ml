//! Train a 2–4–1 tanh model; hidden features are frozen in the learner baseline.
use crate::{
...
pub fn gradient(model: &Model, data: &[Example]) -> std::result::Result<Gradient, &'static str> {
    ...
    for &(features, target) in data {
        ...
        for (unit, &activation) in hidden.iter().enumerate() {
            ...
            // Working fixed-feature baseline. Extend the backward pass to
            // input weights and hidden biases, using the old output weights.
        }
...
fn backward() {
    ...
    for (var sample = 0u; sample < config.samples; sample += 1u) {
        ...
        for (var unit = 0u; unit < HIDDEN; unit += 1u) {
            ...
            // Fixed hidden features baseline: only the output layer learns.
            // Extend this whole backward stage with hidden/input gradients.
