...
fn quantize(
    ...
    // LEARNER: replace global int8 scaling with per-row int8 / packed int4.
    // Supplied shape validation stays above; storage and decoding must agree.
    let maximum = weights.iter().fold(0.0_f32, |m, x| m.max(x.abs()));
    let scale = if maximum == 0. {
        1.
    } else {
        (maximum / 127.).max(f32::MIN_POSITIVE)
    };
    let scales = vec![scale; out_features];
    let data = weights
        .iter()
        .map(|x| (x / scale).round().clamp(-127., 127.) as i8 as u8)
        .collect();
...
impl Quantized {
    fn value(&self, index: usize) -> i8 {
        // Baseline has one signed int8 code per byte.
...
fn experiment() -> Result<(), Box<dyn std::error::Error>> {
    println!("Baseline uses global-scale int8 for both requested precisions; build per-row packed storage.");
    ...
    for bits in [8, 4] {
        ...
        let (rmse, max_abs) = quantization_error(&reference, &output)?;
        println!(
            "requested int{bits}, int8-only baseline: output={output:?}, bytes={}, rmse={rmse:.6}, max_abs={max_abs:.6}",
            q.storage_bytes()
        );
    }
    decoder_comparison()?;
