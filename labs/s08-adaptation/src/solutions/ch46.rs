...
fn train_reward_model(
    ...
    if pairs.is_empty()
        || pairs
            ...
            .any(|x| !x.is_finite())
    {
        return Err("reward model needs finite preference features");
    }
    let mut weights = [0.; 2];
    for _ in 0..steps {
        let mut gradient = [0.; 2];
        for (chosen, rejected) in pairs {
            let difference = [chosen[0] - rejected[0], chosen[1] - rejected[1]];
            let margin = weights[0] * difference[0] + weights[1] * difference[1];
            let slope = -1. / (1. + margin.exp());
            for i in 0..2 {
                gradient[i] += slope * difference[i] / pairs.len() as f64;
            }
        }
        for i in 0..2 {
            weights[i] -= 0.2 * gradient[i];
        }
    }
    Ok(weights)
}
