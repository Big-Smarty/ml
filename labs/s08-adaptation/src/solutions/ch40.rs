...
impl<'a> CachedDecoder<'a> {
    ...
    fn cached_attention(&self, layer: usize, q: &[f32]) -> Vec<f32> {
        ...
        for h in 0..c.heads {
            ...
            for cached_position in 0..positions {
                let dot: f32 = (0..head_dim)
                    .map(|j| {
                        q[h * head_dim + j]
                            * self.layers[layer].keys[cached_position * c.width + h * head_dim + j]
                    })
                    .sum();
            ...
            for (cached_position, score) in scores.into_iter().enumerate() {
                ...
                for j in 0..head_dim {
                    context[h * head_dim + j] += probability
                        * self.layers[layer].values[cached_position * c.width + h * head_dim + j];
                }
            }
        }
        context
...
fn sample_token(
    ...
    token_ids.last().copied().ok_or("no candidate token")
}
...
fn stage_diagnostics() -> Result<(), Box<dyn Error>> {
    ...
    let mut error = 0_f32;
    for end in 1..=tokens.len() {
        let row = cache.step(tokens[end - 1])?;
        let full = model.forward(&tokens[..end])?;
