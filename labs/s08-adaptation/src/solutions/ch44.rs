...
fn exact_top(query: &[f32], docs: &[Vec<f32>]) -> Result<usize, &'static str> {
    ...
    {
        return Err("exact index needs nonempty equal finite vectors");
    }
    let scores: Vec<_> = docs.iter().map(|d| dot(query, d)).collect();
    if scores.iter().any(|x| !x.is_finite()) {
        return Err("index dot product overflow");
    }
    Ok(argmax(&scores))
}
...
fn coverage_risk(rows: &[(f32, bool)], threshold: f32) -> Result<(f32, Option<f32>), &'static str> {
    if rows.is_empty() || !threshold.is_finite() || rows.iter().any(|(s, _)| !s.is_finite()) {
        return Err("evaluation needs finite nonempty rows");
    }
    let answered = rows.iter().filter(|(s, _)| *s >= threshold).count();
    let wrong = rows
        .iter()
        .filter(|(s, correct)| *s >= threshold && !correct)
        .count();
    Ok((
        answered as f32 / rows.len() as f32,
        (answered > 0).then(|| wrong as f32 / answered as f32),
    ))
}
