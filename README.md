...
Install Rust using the [official instructions](https://rust-lang.org/tools/install/). From this repository's root:
...
A failing goal check reports what your implementation has not yet achieved. Ordinary `cargo test` verifies the supplied baseline, plumbing and completed solutions; it does not grade your learning. After trying the hints, read the separate explained solution under `src/solutions/`:
...
With [just](https://just.systems/man/en/) installed:
...
`just lab` uses Cargo's offline mode. Chapters with external dependencies require a one-time explicit `cargo fetch --manifest-path labs/PACKAGE/Cargo.toml` while online; each section README names any dependencies. No lab command downloads datasets. Default runs use tiny bundled fixtures. GPU execution, larger downloads and extended training require the flags described in the relevant chapter. Start with `just gpu 29` for the explicit GPU path after its setup; missing hardware is reported as unsupported, not a passing hardware test.
...
For an offline copy, use the guide's **Download the course** link and unzip it. While online, install Python and the requested Rust toolchain, then fetch external dependencies for any sections you plan to use. The archive does not include compilers or Cargo's dependency cache. Once those are available locally, run `python3 tools/serve.py` inside the extracted `first-principles` folder. The guide and bundled experiments work offline; external readings and optional dataset downloads need a network.
