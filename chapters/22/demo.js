(() => {
  ...
  function render() {
    ...
    for (const item of items) {
      ...
      tr.append(label);
      for (const value of [vector(item.vector), scalar(item.dot.toFixed(3)), scalar(item.cosine.toFixed(3)), scalar(item.distance.toFixed(3))]) {
