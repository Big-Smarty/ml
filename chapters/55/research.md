...
- The fixture computes `output = [1.15, 0.65]`, mean squared loss `0.6725`, bias gradient `[0.15, 1.15]`, and Burn-layout weight gradient `[[0.225, 1.725], [-0.3, -2.3]]` up to f32 rounding. The updated Burn-layout weight is `[[0.1775, 0.5275], [-0.37, 0.33]]`.
...
- The project’s `type Cpu = Autodiff<NdArray<f32>>`, `require_grad()`, `loss.backward()`, and `weight.grad(&gradients)` therefore use the documented API. The direct tensor subtraction is an intentionally transparent SGD step; it does not demonstrate Burn optimizer state or a full `Module` training loop.
...
- Burn’s record system is backend independent within Burn. The 0.21 API defines `NamedMpkBytesRecorder<S>` as an in-memory Named MessagePack recorder with `Vec<u8>` output/input; `FullPrecisionSettings` means f32 floating point and i32 integer serialization. The project correctly records the inner `NdArray<f32>` tensor and loads it with the same recorder and a target device. [NamedMpkBytesRecorder, Burn 0.21.0](https://burn.dev/docs/burn/record/struct.NamedMpkBytesRecorder.html), [Burn record guide](https://burn.dev/books/burn/building-blocks/record.html)
- “Interchange” must be scoped: this bytes record is Burn’s format and is not a PyTorch `.pt`, JAX checkpoint, ONNX model, or generic wire format. Cross-framework interchange needs a separately specified format, tensor names, shape/layout map, dtype policy, preprocessing, and numerical tolerance. The current example serializes one updated tensor, not a model architecture, optimizer state, RNG state, or training cursor.
...
- PyTorch’s official tutorial recommends saving a `state_dict`, recreating the model, then loading it; its device mapping and `eval()` requirements are part of the runtime contract. This supports teaching that weights alone do not define a portable model. [PyTorch saving/loading](https://docs.pytorch.org/tutorials/beginner/saving_loading_models.html)
...
- Mean squared loss matters: with two outputs, `mean((output-target)^2)` gives `dL/doutput = output-target`; a sum reduction or a different batch denominator changes every gradient.
...
The final optional test runs real parity assertions, updates and records BOTH weight and bias, and checks restored inference. The standard-library executable exports the actual trained affine artifact, and the Burn import CLI validates it and compares three input probes. Earlier single-tensor description above is superseded. All optional computation is isolated in projects/ch55/framework with Burn=0.21.0. Additional primary verification: https://onnx.ai/onnx/intro/concepts.html resolves and supports the narrow graph-format explanation; no ONNX exporter is implemented.
