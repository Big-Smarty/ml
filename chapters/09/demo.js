(() => {
  ...
  function render(){
    ...
    if(stage.value==='matrix'){
      out.innerHTML=`${formula.xs} multiplied by ${number(s)}; ${formula.ws} and bias unchanged. ${formula.ys} has values ${vector(dense(s))}. Flat ${formula.xoffset} is offset ${formula.offset}; ${formula.woffset} is offset ${number(2)}.`;
      body.innerHTML=table('Output '+formula.output+': three products plus bias '+number(.5),[formula.i,formula.xi,formula.wi,'Product'],[1,2,3].map((x,i)=>[i,s*x,[1,0,-1][i],s*x*[1,0,-1][i]]));
      ...
      out.innerHTML=`Synthetic block input ${formula.inputshape}, weights ${formula.weightshape}, scores ${formula.scoreshape}. Predicted class ${number(r.prediction)}. Probabilities sum to ${number(r.probabilities.reduce((a,b)=>a+b,0))}. Pixel rows: ${math('<mrow><mo>[</mo><mtable>'+Array.from({length:5},(_,i)=>'<mtr>'+r.pixels.slice(i*3,i*3+3).map(v=>'<mtd>'+scalar(v)+'</mtd>').join('')+'</mtr>').join('')+'</mtable><mo>]</mo></mrow>')}.`;
      grid.innerHTML='<svg width="180" height="235" viewBox="0 0 180 235" role="img" aria-labelledby="digit-grid-title digit-grid-desc"><title id="digit-grid-title">Synthetic 5 by 3 digit pixel grid</title><desc id="digit-grid-desc">Each square is one row-major pixel; its printed value is the model input. Rows and columns map to fifteen features, not handwriting recognition.</desc>'+r.pixels.map((v,i)=>{const row=Math.floor(i/3),col=i%3;const shade=Math.round(255*(1-Math.min(1,v/2)));return `<rect x="${col*55+5}" y="${row*44+5}" width="50" height="39" fill="rgb(${shade},${shade},${shade})" stroke="currentColor"/><text x="${col*55+14}" y="${row*44+30}" fill="${shade>116?'#000':'#fff'}" font-size="15">${CourseNumbers.text(v)}</text>`;}).join('')+'</svg>';
      body.innerHTML=table('Hand-built prototype scores and maximum-shifted softmax',['Class','Score','Probability'],r.logits.map((z,c)=>[c,z,r.probabilities[c]]));
      ...
      body.innerHTML=table('Two complete state updates',['Step','Gradient','Velocity','Momentum '+formula.theta,'Adam '+formula.m,'Adam '+formula.v,'Adam '+formula.theta],optimizer(s).map(r=>[r.t,r.g,r.velocity,r.momentum,r.m,r.v,r.adam]));
