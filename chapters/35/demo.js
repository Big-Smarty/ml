(() => {
  ...
  function draw() {
    ...
    const output = weights.reduce((n, p, i) => n + p * [2, 6, 20][i], 0);
    get('attention-readout').textContent = `Weights [${weights.map(p => p.toFixed(6)).join(', ')}] · weighted value ${output.toFixed(6)}`;
  }
