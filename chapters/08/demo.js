(() => {
  ...
  function render(){
    ...
    root.querySelector('svg').innerHTML=`<title id="tape-title">Shared scalar graph at reverse step ${step}</title><desc id="tape-desc">x has value ${CourseNumbers.text(values[0])} and adjoint ${CourseNumbers.text(gradients[0])}. Its two multiplication uses each contribute ${CourseNumbers.text(step===2?x:0)}; the direct addition path contributes ${CourseNumbers.text(step>0?1:0)}. m has value ${CourseNumbers.text(values[1])} and adjoint ${CourseNumbers.text(gradients[1])}. y has value ${CourseNumbers.text(values[2])} and adjoint one. The table provides the same values.</desc><defs><marker id="tape-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M0 0L10 5L0 10Z" fill="currentColor"/></marker></defs><path d="M115 83Q170 50 235 83M115 107Q170 140 235 107M355 95H450M80 65Q265 -15 500 65" fill="none" stroke="currentColor" stroke-width="2" marker-end="url(#tape-arrow)"/><text x="140" y="53" fill="currentColor" font-size="12">use 1: +${CourseNumbers.text(step===2?x:0)}</text><text x="140" y="148" fill="currentColor" font-size="12">use 2: +${CourseNumbers.text(step===2?x:0)}</text><text x="275" y="24" fill="currentColor" font-size="12">direct x path: +${CourseNumbers.text(step>0?1:0)}</text>${[55,285,500].map((cx,i)=>`<rect x="${cx-50}" y="65" width="110" height="66" rx="5" fill="none" stroke="currentColor"/><text x="${cx-42}" y="87" fill="currentColor" font-size="13">${['x','m=x×x','y=m+x'][i]}: ${CourseNumbers.text(values[i])}</text><text x="${cx-42}" y="111" fill="currentColor" font-size="13">adjoint: ${CourseNumbers.text(gradients[i])}</text>`).join('')}<text x="12" y="180" fill="currentColor" font-size="12">Arrows show forward dependencies. Reverse contributions flow toward x and add.</text>`;
    root.querySelector('[data-table]').innerHTML='<table><caption>Forward values and current reverse adjoints</caption><thead><tr><th>Node</th><th>Value</th><th>Adjoint</th></tr></thead><tbody>'+[formula.x,formula.mul,formula.add].map((name,i)=>`<tr><td>${name}</td><td>${number(values[i])}</td><td>${number(gradients[i])}</td></tr>`).join('')+'</tbody></table>';
