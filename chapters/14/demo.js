(() => {
  ...
  const settings = {
    ...
    tree: ['Threshold on x (left means x ≤ threshold)', 1, 0, 2, .1],
    kernel: ['RBF gamma', .5, .05, 5, .05],
    projection: ['Direction angle in degrees', 0, 0, 90, 5],
    cluster: ['Center 1 horizontal position (center 2 stays [2, 2])', 0, 0, 2.5, .1]
  };
  const sx = x => 40 + 110 * x;
  ...
  function render() {
    ...
    if (!Number.isFinite(v) || v < spec[2] || v > spec[3]) { output.textContent = `Enter a value from ${spec[2]} through ${spec[3]}.`; return; }
    let evidence = [];
    let extra = '';
    let explanation = '';
    if (view.value === 'distance') {
      evidence = points.map((p, i) => [names[i], ((p[0] - 1) * v) ** 2 + (p[1] - 1) ** 2]);
      explanation = `All four squared distances are ${(v * v + 1).toFixed(4)}. The symmetric query remains tied at every horizontal scale; named order selects A for k=1. Scaling can change distances without changing this particular ranking.`;
    } else if (view.value === 'tree') {
      evidence = points.map((p, i) => [names[i], p[0] <= v ? 'left' : 'right']);
      explanation = `Query x=1 goes ${1 <= v ? 'left' : 'right'} at threshold ${v}. At threshold 1 each leaf contains one positive and one negative corner, so each Gini impurity is 0.5. A second split is needed for XOR.`;
      ...
      explanation = `Squared distance is 2 for every corner, so RBF similarity exp(−${v}×2)=${Math.exp(-2 * v).toFixed(4)}. This changes influence width; it does not fit kernel coefficients or separate this tied query.`;
      ...
      evidence = [['Direction x', q[0]], ['Direction y', q[1]], ['Projection coordinate', score], ['Reconstruction x', reconstructed[0]], ['Reconstruction y', reconstructed[1]], ['Squared lost norm', loss]];
      explanation = `Unit direction at ${v} degrees projects query [1,1] to coordinate ${score.toFixed(4)} and reconstructs [${reconstructed.map(x => x.toFixed(4)).join(', ')}]. Squared lost norm ${loss.toFixed(4)}. Origin is fixed for this calculation; fitted PCA also subtracts its training mean.`;
      ...
      explanation = `Centers are [${v},0] and [2,2]. Query distances are ${d1.toFixed(4)} and ${d2.toFixed(4)}; center ${d1 <= d2 ? '1' : '2'} wins${d1 === d2 ? ' by the fixed tie rule' : ''}. This is an assignment calculation; the Rust learner must also update means.`;
