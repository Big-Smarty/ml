...
The active learner route is `just lab 49` → edit `labs/s09-advanced/src/ch49.rs` → `just lab-check 49`. The lesson now contains 2 sessions and 6 ordered question-led steps. Goal: Implement affine prefix scan and normalized feature-kernel attention with direct-oracle and causal checks. The separately explained solution is `labs/s09-advanced/src/solutions/ch49.rs`; supplied validation, fixtures, and I/O are in `src/common/ch49.rs`. Original `projects/ch49` implementations remain preserved reference evidence. Earlier workflow descriptions below are historical reference notes and do not replace this active route.
