(() => {
  ...
  function render() {
    ...
    if (!calibration) {
      ...
      values.sort((a, b) => a - b);
      output.innerHTML = `Seed ${seed}; ${unit.value === 'row' ? '18 paired rows' : '6 whole machines'} per replicate; 2000 replicates. Original accuracy ${scalar(accuracy(rows))}. Percentile endpoints ${interval(values[50], values[1950])}. First replicate accuracy ${scalar(first)}.`;
      ...
      if (!Number.isFinite(raw) || raw < .5 || raw > 3) { output.innerHTML = `Enter a confidence multiplier from ${scalar(.5)} through ${scalar(3)}.`; return; }
