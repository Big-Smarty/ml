...
impl Dense {
    ...
    fn loss_and_gradient(&self, input: [f32; 2], target: [f32; 2]) -> (f32, Gradient) {
        ...
        let gradient = Gradient {
            weights: std::array::from_fn(|o| std::array::from_fn(|i| error[o] * input[i])),
            bias: error,
        };
        (loss, gradient)
    }

    fn apply_sgd(&mut self, gradient: &Gradient, learning_rate: f32) -> Result<(), &'static str> {
        ...
        if self
            ...
            .any(|(parameter, derivative)| {
                !parameter.is_finite()
                    || !derivative.is_finite()
                    || !(parameter - learning_rate * derivative).is_finite()
            })
        {
        ...
        for o in 0..2 {
            for i in 0..2 {
                self.weights[o][i] -= learning_rate * gradient.weights[o][i];
            }
            self.bias[o] -= learning_rate * gradient.bias[o];
        }
        Ok(())
    }
    ...
    fn save(&self, path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for value in self.weights.iter().flatten().chain(&self.bias) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (loss, gradient) = model.loss_and_gradient(INPUT, TARGET);
    println!("scratch output: [{:.6}, {:.6}]", output[0], output[1]);
    println!("scratch mean squared error: {loss:.6}");
    println!("scratch weight gradient: {:?}", gradient.weights);
    model.apply_sgd(&gradient, LEARNING_RATE)?;
    println!("scratch weights after SGD: {:?}", model.weights);
