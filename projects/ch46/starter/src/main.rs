//! Enumerate Chapter 45's policy-gradient expectation, then complete the DPO TODO.
...
fn reinforce_exact_step(logits: &mut [f64; 2], rewards: [f64; 2], rate: f64) {
    let p = softmax2(*logits);
    let expected = p[0] * rewards[0] + p[1] * rewards[1];
    for i in 0..2 {
        logits[i] += rate * p[i] * (rewards[i] - expected);
    }
}

fn dpo_pair_loss(policy_margin: f64, reference_margin: f64, beta: f64) -> f64 {
    // TODO: form the relative margin, then evaluate stable softplus of its negative.
    let _ = (policy_margin, reference_margin, beta);
    todo!("compute -ln(sigmoid(beta * (policy_margin - reference_margin)))")
}
...
fn main() {
    ...
    println!("previous checkpoint policy: {:.3?}", softmax2(logits));
...
mod tests {
    ...
    #[test]
    fn equal_policy_and_reference_margins_have_log_two_loss() {
